...
class Model():
	def __init__(self,batch_size, hidden_size, layers,
		...
		att_entity = self.get_attentive_entity(associate_entity, self.user_input, self.item_input,self.entity_embedding)

		sim3 = tf.reduce_sum(tf.multiply(user_latent, att_entity), axis=1, keep_dims = True)
		vector3 = tf.concat([user_latent, att_entity,sim3, tf.multiply(user_latent, att_entity) ], axis=1)
		vector3 = tf.layers.batch_normalization(vector3)

		for i in range(len(layers)):
			hidden = Dense(layers[i], activation='relu',kernel_initializer = 'lecun_uniform',name='v1_att_hidden_' + str(i))
			vector3 = hidden(vector3)
		self.ue_logits = Dense(1, kernel_initializer='lecun_uniform', name = 'ue_pred')(vector3)


		self.ui_logits += self.ue_logits
		...
		# self.cost += 0.01*self.regularization(self.user_input, self.entity_embedding)
		# self.cost += 0.1*self.regularization(self.user_input, self.entity_embedding)

		self.train_op =  tf.train.AdamOptimizer(learning_rate=0.0001).minimize(self.cost)


	def regularization(self, user_input, entity_embedding):
		n_neighbor = np.load('train_s_neighbor.npy')
		user_neighbor = tf.nn.embedding_lookup(n_neighbor, user_input)
		user_neighbor_embedding = tf.nn.embedding_lookup(entity_embedding, user_neighbor)
		user_neighbor_embedding = tf.reduce_mean(user_neighbor_embedding, axis=1)
		user_embedding = tf.nn.embedding_lookup(entity_embedding, user_input)
		return tf.reduce_sum(tf.square(user_embedding - user_neighbor_embedding))

	def get_attentive_entity(self,associate_entity, user_input,item_input, entity_embedding):
		num_asso = associate_entity.shape[-1]
		asso_entity = tf.nn.embedding_lookup(associate_entity, item_input)
		asso_embedding = tf.nn.embedding_lookup(entity_embedding, asso_entity) ## size (batch,n,embedding_size)

		user_embedding = tf.nn.embedding_lookup(entity_embedding, user_input)
		embedding_size = user_embedding.shape.as_list()[-1]

		expand_user_latent = tf.expand_dims(user_embedding, 1) ##size (batch,1,embedding_size)

		hybrid_asso_embedding = expand_user_latent*asso_embedding


		units = int(embedding_size/2)
		layer_item_1 = Dense(units, activation='relu',kernel_initializer = 'lecun_uniform',name='att1layer_item_1')
		layer_item_2 = Dense(1, kernel_initializer = 'lecun_uniform', name='att1layer_item_2')
		attentions = layer_item_2(layer_item_1(hybrid_asso_embedding))
		attentions = tf.reshape(attentions, [-1, num_asso])

		self.att = tf.nn.softmax(10*attentions)

		atts = tf.expand_dims(self.att, -1) ##size (batch, n ,1)
		att_entity = tf.reduce_sum(atts*asso_embedding, axis=1)
